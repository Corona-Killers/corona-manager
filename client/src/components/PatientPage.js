...
function PatientPage() {
  ...
  return (
    <div id="patient">
      ...
      <select
        onChange={async (e) => {
          await read(`patients/byId/${e.target.value}`).then((r) => {
            setDisplay(r);
          });
        }}
      >
      ...
      {display !== 0 && (
        <div>
          <div className="field">
            Patient Name: <span>{display.name}</span>
          </div>
          <div className="field">
            Date of birth: <span>{display.dateOfBirth}</span>
          </div>
          <div className="field">
            City: <span>{display.City.name}</span>
          </div>
          {display.CovidTests[0] && (
            <div className="field">
              Covid Test Result:{" "}
              <span>{display.CovidTests[0].isSick ? "sick" : "healthy"}</span>
            </div>
          )}
          <div className="field">Symptoms:</div>
          <ul>
            {display.SymptomsByPatients.map((e) => (
              <li>{e.Symptom.name}</li>
            ))}
          </ul>
        </div>
      )}
